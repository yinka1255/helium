	...
	<div class="main-content">
		...
		<div class="row">
			<div class="col-xs-12">
				...
				<br/><br/>
				<form role="form" method="post" action="{{url('admin/update_session')}}" class="form-horizontal ">
					{{ csrf_field() }}
					<input type="hidden" name="id" value="{{$session->id}}" />
					<div class="form-group">
						<label for="field-2" class="col-sm-2 control-label">Name</label>
						<div class="col-sm-5">
							<input type="text" name="name" class="form-control" value="{{$session->name}}" placeholder="Name" required>
						</div>
					</div>
					<div class="form-group">
						<div class="col-sm-offset-2 col-sm-5">
							<button type="submit" class="btn btn-default">Update session</button>
						</div>
					</div>
				</form>
			</div>
